...
BRKGA< Decoder, RNG >::BRKGA(unsigned _n, unsigned _p, double _pe, double _pm, double _rhoe,
		...
		pe(unsigned(_pe * p)), pm(unsigned(_pm * p)), rhoe(_rhoe), refRNG(rng),
...
void BRKGA< Decoder, RNG >::evolve(unsigned generations) {
	...
	for(unsigned i = 0; i < generations; ++i) {
		for(unsigned j = 0; j < K; ++j) {
			//(this->*pEvolution)(*current[j], *previous[j]);	// First evolve the population (curr, next)
...
template< class Decoder, class RNG >
bool BRKGA<Decoder, RNG>::isRepeated(const std::vector< double >& chrA, const std::vector< double >& chrB) const {
	return (chrA == chrB);
}
